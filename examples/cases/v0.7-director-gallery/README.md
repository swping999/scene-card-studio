# v0.7 Director Gallery · After-only

This gallery is intentionally After-only for public presentation. The four images demonstrate the new visual mechanisms rather than a generic filter menu. The source photos and semantic Scene Cards remain private to the generation session; the records below preserve the generated asset hash and the exact profile used.

| After | Narrative System | Expression Profile | Mechanism |
| --- | --- | --- | --- |
...
Each new Profile now has three visibly different subjects so the gallery demonstrates general behavior rather than one memorized composition.

| Profile | Example 1 | Example 2 | Example 3 |
| --- | --- | --- | --- |
...
本画廊对外只展示 After，不把 Before 放在首页。四套新增视觉语言现在每套提供 3 张不同题材的成片，证明它们不是猫、室内或某一种构图专用。国风诗意案例覆盖宠物与室内、建筑与风景、物件与风景；标题和诗句来自本地 Manifest，并由 SVG 确定性排版。

这四组案例均为仓库原创示例，只使用通用视觉机制，不模仿具名艺术家、工作室、出版物或第三方项目的受保护素材与版式。
