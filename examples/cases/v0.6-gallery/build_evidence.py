...
from moments_to_pages.model import Direction, Interpretation, Observation, TransformationPolicy
from moments_to_pages.prompt_compiler import compile_manifest
...
def build_payloads() -> dict[Path, bytes]:
    ...
    for case in records["cases"]:
        ...
        manifest = compile_manifest(
            [card], case["system"], expression_profile=case["expression_profile"],
            source_root=evidence_dir, story_path="story.json",
            reference_outputs=[Path(os.path.relpath(after, evidence_dir)).as_posix()],
        )
        manifest_bytes = _json_bytes(manifest)
        payloads[evidence_dir / "story.json"] = story_bytes
        payloads[evidence_dir / "prompt-manifest.json"] = manifest_bytes
        index_cases.append({
            "case_id": case_id, "display_name": case["display_name"], "system": case["system"],
            "expression_profile": case["expression_profile"], "story": f"{case_id}/story.json",
    ...
    payloads[GALLERY / "evidence/index.json"] = _json_bytes({
        "schema_version": "1.0", "source_record": "../case-records.json",
        "purpose": "Recompilable Scene Cards and Prompt Manifests for the landscape-led v0.6 gallery.",
        "cases": index_cases,
    })
    return payloads
